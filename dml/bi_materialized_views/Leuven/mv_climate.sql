...
-- mv_bi_leuven_daily_avg_precipitation_projection
CREATE MATERIALIZED VIEW mv_bi_leuven_daily_avg_precipitation_projection AS
 select
...
-- mv_bi_leuven_avg_precipitation_projection 
CREATE MATERIALIZED VIEW mv_bi_leuven_avg_precipitation_projection AS
select 
  x.scenario,
  case
...
from mv_bi_leuven_daily_avg_precipitation_projection bi
group by 
...
-- mv_bi_leuven_daily_avg_temperature_projection
CREATE MATERIALIZED VIEW mv_bi_leuven_daily_avg_temperature_projection AS
 select
    date_part('year', date_from) as anno,
    to_char(date_from, 'MM') || '-' || trim(to_char(date_from, 'TMMonth')) as mese,
    date_trunc('day', date_from)::date as giorno,
...
-- mv_bi_leuven_avg_temperature_projection
CREATE MATERIALIZED VIEW mv_bi_leuven_avg_temperature_projection AS
select 
  x.scenario,
  case
    when anno between 1985 and 2014 then '1985–2014'
...
from mv_bi_leuven_daily_avg_temperature_projection bi
group by 
  bi.measure_cod_scenario,
  giorno,
  anno
...
;
 ...
 CREATE MATERIALIZED VIEW mv_bi_leuven_daily_temperature_actual AS
 select
    date_part('year', date_from) as anno,
    to_char(date_from, 'MM') || '-' || trim(to_char(date_from, 'TMMonth')) as mese,
    date_trunc('day', date_from)::date as giorno,
...
CREATE MATERIALIZED VIEW mv_bi_leuven_daily_avg_precipitation_actual AS
select 
date_part('year', giorno) as year,
to_char(giorno, 'MM') || '-' || trim(to_char(giorno, 'TMMonth')) as month,
date_trunc('day', giorno)::date as date,
avg(val) as avg_precipitation
from mv_bi_leuven_daily_precipitation_actual bi 
group by     
date_part('year', giorno),
to_char(giorno, 'MM') || '-' || trim(to_char(giorno, 'TMMonth')),
date_trunc('day', giorno)::date;
